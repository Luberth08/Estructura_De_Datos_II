...
class MotorIndexacion:
    ...
    def eliminar_registro(self, id_reg: int) -> bool:
        ...
        # 1. Eliminar del árbol de indexación
        eliminado = self.__arbol.eliminar(id_reg)
        if not eliminado:
            return False
        
        # 2. Marcar el registro como eliminado en el archivo
        self.__marcar_como_eliminado(posicion)
        
        # 3. (Opcional) Compactar el archivo periódicamente
        return True

    def __marcar_como_eliminado(self, posicion: int):
        """Marca un registro como eliminado escribiendo un valor especial"""
        self.__archivo.seek(posicion)
        ...
        # Desempaquetar y modificar el ID a un valor especial (ej: -1)
        campos = struct.unpack(self.FORMATO_REGISTRO, registro)
        registro_modificado = struct.pack(
            self.FORMATO_REGISTRO,
            -1,  # ID especial para registros eliminados
            campos[1],  # Mantener el resto de campos sin cambios
            campos[2],
            campos[3]
        )
        
        # Escribir de vuelta en la misma posición
        self.__archivo.seek(posicion)
        self.__archivo.write(registro_modificado)
        self.__archivo.flush()
